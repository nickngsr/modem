...
var createSession = function() {
    ...
    ussd_session.query = function(string, callback, end) {
        if(end) this.end_time = end;

        var self = this;
        this.modem.execute('ATZ +CFUN=1', function(response, escape_char) {
            setTimeout(function(){
                if(callback)
                    self.once('response', function(a,b,c){
                        self.clearTimeout();
                        self.modem.execute('AT+CUSD=2', function(){
                            callback(a,b,c);
                        },false,self.end_time);
                    });
                ...
                var execute_job = self.modem.execute('AT+CUSD=1,"'+encoded+'",15', function(response, escape_char) {
                    ...
                    if(escape_char && escape_char.match(/error/i)){
    ...
    ussd_session.close = function(callback) {
        ...
        if(!this.closed) {
            ...
            this.modem.execute('AT+CUSD=2', function(){
                if(callback)
                    callback(new Error("Device timed out"));
            },false,this.end_time);
            this.state = 'closed';
            this.emit('close');
        }
    ...
    ussd_session.responseCompleted = function() {
        ...
        if(this.modem.ussd_pdu && args[2] == '15')
            ...
            response = args[1].match(/(..)/g).slice(0).map(function (chr) {return String.fromCharCode (parseInt( chr, 16 ))})
        } else
             response = args[1];
    ...
    ussd_session.encode = function(string) {
        if(ussd_session.modem.ussd_pdu)
            return pdu.encode7Bit (pdu.try7Bit (string));
        else
            return string;
    };
